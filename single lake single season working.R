...
"Compare.plots" <- function()
{
  ...
  mtext(expression(paste("Effort (AD"%.%"ha"^{-1},")")),2,cex=1.2,line=2.5)
  ...
  mtext(expression(paste("CPUE (Fish"%.%"ha"%.%"AD"^{-1},")")),4,cex=1.2,line=3)
...
"Response.metrics" <- function()
{
  ...
  mtext(expression(paste("Value (AD"%.%"ha"^{-1},")")),2,line=2.2)
  par(mfcol=c(1,1))
...
"Value.plot" <- function()
{
  plot(seq(0,5,0.001),(seq(0,5,0.001)/cpue_base-1)^1,t="l",ylim=c(0,5),xaxs="i",yaxs="i",xlab="",ylab="",lwd=2)
  lines(seq(0,5,0.001),(seq(0,5,0.001)/cpue_base-1)^1.5,lty=2,lwd=2)
  lines(seq(0,5,0.001),(seq(0,5,0.001)/cpue_base-1)^0.5,lty=3,lwd=2)
  ...
  legend("topleft",legend=c(expression(paste(beta,"=0.5")),
                            ...
                            expression(paste(beta,"=1.5"))),lty=c(3,1,2),bty="n",cex=1.4,lwd=2)
  mtext(expression(bold("Value (AD)")),2,cex=1.25,line=2.3)
  mtext(expression(bold(paste("CPUE (fish"%.%"ha"%.%"AD"^{-1},")"))),1,cex=1.25,line=2.75)
}
